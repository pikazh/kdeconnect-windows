    ...
    : KdeConnectPlugin(parent, args)
{
    ...
    valid = (hr == S_OK);
    if (!valid) {
        qWarning(KDECONNECT_PLUGIN_SYSTEMVOLUME)
            << "Initialization failed: Failed to create MMDeviceEnumerator";
        char buf[64] = {0};
        sprintf(buf, "Error Code: %lx", hr);
        qWarning(KDECONNECT_PLUGIN_SYSTEMVOLUME) << buf;
    }
}
...
bool SystemvolumePlugin::sendSinkList()
{
    ...
    if (hr != S_OK) {
        qWarning(KDECONNECT_PLUGIN_SYSTEMVOLUME) << "Failed to Enumumerate AudioEndpoints";
        char buf[64] = {0};
        sprintf(buf, "Error Code: %lx", hr);
        qWarning(KDECONNECT_PLUGIN_SYSTEMVOLUME) << buf;
        return false;
    ...
    if (!deviceCount) {
        qWarning(KDECONNECT_PLUGIN_SYSTEMVOLUME) << "No audio devices detected";
        return false;
    }
    ...
    for (unsigned int i = 0; i < deviceCount; i++) {
        ...
        if (hr != S_OK) {
            qWarning(KDECONNECT_PLUGIN_SYSTEMVOLUME)
                << "Failed to create IAudioEndpointVolume for device:" << name;
            char buf[64] = {0};
            sprintf(buf, "Error Code: %lx", hr);
            qWarning(KDECONNECT_PLUGIN_SYSTEMVOLUME) << buf;
...
void SystemvolumePlugin::onPluginEnabled()
{
    if (!valid)
        return;
...
HRESULT SystemvolumePlugin::setDefaultAudioPlaybackDevice(QString &name, bool enabled)
{
    ...
    for (auto &entry : idToNameMap.toStdMap()) {
        if (entry.second == name) {
            deviceId = new WCHAR[entry.first.length() + 1];
            memset(deviceId, 0, sizeof(WCHAR) * (entry.first.length() + 1));
            wcscpy_s(deviceId, entry.first.length() + 1, entry.first.toStdWString().data());
            break;
        }
    }
