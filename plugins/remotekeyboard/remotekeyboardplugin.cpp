...
void RemoteKeyboardPlugin::receivePacket(const NetworkPacket &np)
{
    if (np.type() == PACKET_TYPE_MOUSEPAD_ECHO) {
        ...
        Q_EMIT keyPressEchoReceived(np.get<QString>(QStringLiteral("key")),
...
void RemoteKeyboardPlugin::sendKeyPress(
...
void RemoteKeyboardPlugin::sendQKeyEvent(int key,
                                         Qt::KeyboardModifiers modifiers,
                                         const QString &text,
                                         bool sendAck)
{
    int internalKey = translateQtKey(key);
    QString t = text;
    ...
    if ((modifiers & Qt::ControlModifier) != 0 && !text.isEmpty() && !text[0].isLetterOrNumber()
        && text != QLatin1String(" ")) {
        t = QKeySequence(key).toString().toLower();
    }
...
int RemoteKeyboardPlugin::translateQtKey(int qtKey)
{
