...
public:
    PluginMetaDataPrivate(const QJsonObject &obj, const QString &filePath)
        ...
        , m_filePath(filePath)
        , m_pluginId(QFileInfo(filePath).completeBaseName())
    {
...
QList<PluginMetaData> PluginMetaData::load()
{
    ...
    PluginMetaDataPrivate::forEachPlugin([&](const QFileInfo &fileInfo){
        ...
        if(!metaData.isValid())
        {
            qWarning(KDECONNECT_COREADDONS) << filePath << " doesn't contain valid JSON metadata.";
            return;
        }
...
QString PluginMetaData::pluginId() const
{
    return d->m_pluginId;
}
...
bool PluginMetaData::isEnabledByDefault() const
{
    ...
    if (val.isBool()) {
        ...
        qWarning(KDECONNECT_COREADDONS) << "Expected JSON property " << key
                                        << " in " << d->m_filePath
...
QStringList PluginMetaData::value(QStringView key, const QStringList &defaultValue) const
{
    ...
    else
    {
        ...
        qWarning(KDECONNECT_COREADDONS) << "Expected JSON property " << key
                                        << "to be a string list in " << d->m_filePath
...
int PluginMetaData::value(QStringView key, int defaultValue) const
{
    ...
    else if (value.isString())
    {
        ...
        {
            qWarning(KDECONNECT_COREADDONS) << "Expected " << key
                                            << " to be an int, instead " << intString
                                            << " was specified in " << d->m_filePath;
            return defaultValue;
        }
    }
    else
