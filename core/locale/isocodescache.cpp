...
static QString isoCodesPath(QStringView file)
{
    ...
    auto path = QStandardPaths::locate(QStandardPaths::GenericDataLocation,
                                       QLatin1String("iso-codes/json/") + file.toString(),
    ...
    for (const char *installLocation : {"/share", "/bin/data"}) {
        path = QStringLiteral(ISO_CODES_PREFIX) + QString(installLocation)
               + QStringLiteral("/iso-codes/json/") + file.toString();
...
*/
    ...
    return QLatin1String(":/org.kde.ki18n/iso-codes/cache/") + file.toString();
}
...
{
    return cachePath() + file.toString();
}
...
static std::unique_ptr<QFile> openCacheFile(QStringView cacheFileName, QStringView isoCodesFileName)
{
    ...
    if (!jsonFi.exists()) { // no source file means we can only use an embedded cache
        auto f = std::make_unique<QFile>(QStringLiteral(":/org.kde.ki18n/iso-codes/cache/")
...
{
    qCDebug(KDECONNECT_CORE) << "Rebuilding ISO 3166-1 cache";

    QFile file(isoCodesPath);
    if (!file.open(QFile::ReadOnly)) {
        qCWarning(KDECONNECT_CORE)
            << "Unable to open iso_3166-1.json" << isoCodesPath << file.errorString();
        return;
    }
    ...
    if (!cache.open(QFile::WriteOnly)) {
        qCWarning(KDECONNECT_CORE)
            << "Failed to write ISO 3166-1 cache:" << cache.errorString() << cache.fileName();
        return;
    }
...
{
    qCDebug(KDECONNECT_CORE) << "Rebuilding ISO 3166-2 cache";
    QFile file(isoCodesPath);
    if (!file.open(QFile::ReadOnly)) {
        qCWarning(KDECONNECT_CORE)
            << "Unable to open iso_3166-2.json" << isoCodesPath << file.errorString();
        return;
    }
    ...
    if (!cache.open(QFile::WriteOnly)) {
        qCWarning(KDECONNECT_CORE)
            << "Failed to write ISO 3166-2 cache:" << cache.errorString() << cache.fileName();
        return;
    }
